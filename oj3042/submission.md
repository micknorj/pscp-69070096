...
```text
Input:
บรรทัดแรก ตัวเลข (int)

Output:
ตัวเลขปัดลงหลักสิบ ลดลงทีละ 10 จนถึง 0 ในบรรทัดเดียว เว้นวรรคด้วยช่องว่าง

Constraints:
รับตัวเลขจำนวนเต็มบวก
```
...
```text
Step 1: รับค่า number เป็น int
Step 2: ปัดลงหลักสิบ (number // 10) * 10
Step 3: ใช้ loop วนลดค่าทีละ 10 ใส่ List จนถึง 0
Step 4: Print ค่าใน List ออกมาด้วย unpacking *
```
...
```text
เหมือนแผนแรก ใช้ (number // 10) * 10 เพื่อปัดเศษลงหลักสิบทันที แล้วใช้ while loop วนลดทีละ 10 เก็บเข้า List แล้ว Print ออกมาในบรรทัดเดียว
```
