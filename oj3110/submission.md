...
```text
Input:
รับจุดเริ่มต้น จุดปลายทาง และน้ำหนัก

Output:
ค่าจัดส่งทศนิยม 2 ตำแหน่ง หรือ Error

Constraints:
เส้นทางต้องมีอยู่ใน ROUTES
```
...
```text
Step 1: รับค่า front end และ weight
Step 2: เช็กเส้นทางด้วย if elif
Step 3: ใช้ math.ceil() ปัด weight ขึ้นเป็นจำนวนเต็ม
Step 4: คำนวณค่าจัดส่งจาก base + weight * rate ตามแต่ละเส้นทาง
Step 5: Print ค่าจัดส่งทศนิยม 2 ตำแหน่ง หากไม่มีเส้นทางให้ Print "Error"
```
...
```text
เปลี่ยนจากแผนแรกที่ใช้ if elif เช็กเส้นทาง มาใช้ Dict เก็บ route พร้อม base และ rate แล้วเช็ก front กับ end ใน ROUTES ถ้ามีให้คำนวณ total จาก base + weight * rate แล้ว Print ผลออกมา นอกนั้น Print "Error"
```
...
```text
เส้นทางที่ไม่มีในเงื่อนไขที่กำหนด
```
